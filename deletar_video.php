...
// Buscar o arquivo recursivamente
$videos_path = "evidencias/" . $cidade . "/Videos";

function encontrarArquivo($dir, $nome) {
    if (!is_dir($dir)) {
        return false;
    }
    ...
    foreach ($iterator as $file) {
        if ($file->isFile() && $file->getFilename() === $nome) {
...
$file_path = encontrarArquivo($videos_path, $nome_arquivo);
...
// Deletar arquivo
if (unlink($file_path)) {
    echo json_encode(['success' => true, 'message' => 'Vídeo deletado com sucesso']);
} else {
    echo json_encode(['success' => false, 'message' => 'Erro ao deletar vídeo']);
}
?>
